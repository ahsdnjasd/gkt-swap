...
export default function LPPositionCard({ position, poolStats }: LPPositionCardProps) {
  ...
  return (
    <div className="bg-card border border-cyan/30 rounded-[2.5rem] p-8 shadow-[0_0_20px_rgba(0,212,255,0.05)]">
      <div className="flex items-center gap-3 mb-8">
        <div className="bg-cyan/20 p-3 rounded-2xl text-cyan">
          <Layers size={24} />
        </div>
        <h3 className="text-white font-display font-black text-xl uppercase tracking-tighter">Your Position</h3>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Shares Owned */}
        <div className="bg-background/40 border border-border p-5 rounded-3xl group hover:border-cyan/50 transition-all">
          ...
          <p className="text-white font-mono text-xl font-bold">{formatToken(position.lpShares, 4)}</p>
          <p className="text-cyan text-xs font-mono mt-1">{formatPercent(poolPercentage)} of pool</p>
        </div>
        ...
        <div className="bg-background/40 border border-border p-5 rounded-3xl group hover:border-cyan/50 transition-all">
          ...
          <p className="text-white font-mono text-xl font-bold">{formatToken(xlm)}</p>
          <p className="text-muted/60 text-[10px] font-mono mt-1">Staged in liquidity</p>
        </div>

        {/* LQID Value */}
        <div className="bg-background/40 border border-border p-5 rounded-3xl group hover:border-violet/50 transition-all">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-white font-mono text-xl font-bold">{formatToken(lqid)}</p>
          <p className="text-muted/60 text-[10px] font-mono mt-1">Staged in liquidity</p>
        </div>
      </div>
