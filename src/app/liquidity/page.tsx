  ...
  return (
    <div className="max-w-[1000px] mx-auto space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-display font-black text-foreground">Liquidity Pools</h1>
          <p className="text-muted text-sm font-medium">Provide liquidity and earn trading fees on every swap</p>
        </div>
        
        <div className="flex items-center gap-4 p-4 bg-success/5 border border-success/15 rounded-2xl">
          <div className="bg-success/10 p-2 rounded-xl text-success">
            <Percent size={20} />
          </div>
          <div>
            <p className="text-foreground font-mono font-bold leading-none">0.3% Fee</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-7 space-y-8">
          <div className="glass-strong p-8 rounded-[2.5rem] relative overflow-hidden">
             <div className="absolute top-0 right-0 w-48 h-48 bg-primary/5 blur-[80px] rounded-full" />
             
             <h3 className="text-foreground font-display font-bold text-xl mb-6 flex items-center gap-2">
               Current Pool State <Droplets size={18} className="text-primary" />
             </h3>

             <div className="grid grid-cols-2 gap-8">
               <div className="space-y-1">
                 <span className="text-muted text-[10px] uppercase font-bold tracking-[0.2em]">XLM Locked</span>
                 <p className="text-2xl font-mono font-black text-foreground">{pool ? formatToken(pool.xlmReserve) : '---'}</p>
             ...
             <div className="mt-10 p-5 bg-green-50/40 border border-green-100 rounded-3xl flex justify-between items-center">
               <div className="flex flex-col">
                 <span className="text-[10px] text-muted font-bold uppercase tracking-widest">Global TVL</span>
                 <span className="text-lg font-mono font-bold text-foreground">{pool ? formatXLM(pool.tvlXLM) : '---'}</span>
               </div>
               <div className="flex flex-col text-right">
                 <span className="text-[10px] text-muted font-bold uppercase tracking-widest">Total LP Shares</span>
                 <span className="text-lg font-mono font-bold text-foreground">{pool ? formatToken(pool.totalLPShares, 2) : '---'}</span>
               </div>
             </div>
          </div>

          <div className="space-y-6">
            <h3 className="text-2xl font-display font-black text-foreground flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <div key={idx} className="p-6 glass border border-green-100 rounded-3xl hover:border-primary/20 transition-all">
                  <div className="bg-green-50/80 w-12 h-12 rounded-2xl flex items-center justify-center mb-4 border border-green-100 shadow-sm">
                    {step.icon}
                  </div>
                  <h4 className="text-foreground font-display font-bold text-sm mb-2">{step.title}</h4>
